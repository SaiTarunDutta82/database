...
public class userInformationServices
{
    ...
    {
        string query = "SELECT * FROM UserData WHERE DeviceID = '" + deviceID + "'";
    ...
    private async Task<JObject> CreateUser(string deviceID, string name, int profilePic)
    {
        ...
        string query = $"INSERT INTO UserData (UserID, DeviceID, Name, IsPrimaryUser) " +
               $"VALUES ('{UserID}', '{deviceID}', '{userObject["Name"]}', 1)";
        await dbConnection.ExecuteQueryAsync(query);
        return userObject;
    }
    ...
    public async Task UpdateAge(string userID, int age)
    {
        ...
        string query = $"UPDATE UserData SET Age = '{age}' WHERE UserID = '{userID}'";
        await dbConnection.ExecuteQueryAsync(query);
    }
    ...
    public async Task UpdateMusic(string userID, bool music)
    {
        ...
        string query = $"UPDATE UserData SET IsMusicEnabled = '{musicEnabled}' WHERE UserID = '{userID}'";
        await dbConnection.ExecuteQueryAsync(query);
    }
    ...
    public async Task UpdateSound(string userID, bool sound)
    {
        ...
        string query = $"UPDATE UserData SET IsSoundEnabled = '{soundEnabled}' WHERE UserID = '{userID}'";
        await dbConnection.ExecuteQueryAsync(query);
    }
    ...
    public async Task UpdateProfilePicture(string userID, int profilePic)
    {
        ...
        string query = $"UPDATE UserData SET ProfilePicture = '{profilePic}' WHERE UserID = '{userID}'";
        await dbConnection.ExecuteQueryAsync(query);
    }
