...
public class GameInformationServices
{
   ...
   public async Task<JObject> RetriveUserHighestScore(string userID)
   {
       ...
       if (gameObjects.Count > 0)
       {
           ...
           string query = "SELECT COUNT(*) AS TotalRecords FROM GameStatistics  WHERE userID = '" + userID + "' AND gameCompleted = 1;";
           List<string> gameInfo = await dbConnection.ExecuteQueryAsync(query);
           string Info = gameInfo[0];


           // Parse the Info string into a JObject
           JObject infoObject = JObject.Parse(Info);


           // Access the TotalRecords property and convert it to string before assigning it to gameObject
           string totalRecordsStr = infoObject["TotalRecords"].ToString();


           // Assuming your gameObject is a JObject
           JObject gameObject = JObject.Parse(gameObjects[0]); // Parse the JSON string into a JObject


           // Assign the total records count to the appropriate field in your gameObject
           gameObject["noOfGame"] = totalRecordsStr;


           return gameObject; // Add each game object to the result JArray
           // _logger.LogInformation(Info["TotalRecords"]);
       }
